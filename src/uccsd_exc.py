...
class UCCSD_exc(Operation):
    ...
    def __init__(
        self, weights, wires, s_wires=None, d_wires=None, singles=None, doubles=None, init_state=None, do_queue=True, id=None
    ):
        ...
        # if shape != (len(s_wires) + len(d_wires) + len(singles) + len(doubles),):
        #     raise ValueError(
        ...
        self._hyperparameters = {"init_state": init_state, "s_wires": s_wires, "d_wires": d_wires, "singles": singles, "doubles": doubles}
    ...
    def compute_decomposition(
        weights, wires, s_wires, d_wires, singles, doubles, init_state
        ...
        # weights is (wfn-singles, wfn-doubles, response-singles-A, response-doubles-A)
        # sc: excitation operators before U
        # "TDA", use only A part with a factor sqrt(2),
        for i, double in enumerate(doubles):
            op_list.append(qml.DoubleExcitation(1.41421356237*weights[2*len(s_wires) + len(d_wires) + i], double))
        for i, single in enumerate(singles):
            op_list.append(qml.SingleExcitation(1.41421356237*weights[len(s_wires) + len(d_wires) + i], single))
